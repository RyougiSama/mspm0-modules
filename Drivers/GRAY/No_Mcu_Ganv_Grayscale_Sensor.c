...
void Get_Analog_value(unsigned short *result)
{
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        for (j = 0; j < 8; j++)
        {
            Analag += Get_adc_of_user(); // 累加ADC采样值
        }
        if (!Direction)
            result[i] = Analag / 8; // 计算平均值
        else
            result[7 - i] = Analag / 8; // 计算平均值
        Analag = 0;                     // 重置累加器
    }
}
...
/* 函数功能：获取原始校准数据
   返回值：1-成功 0-未初始化 */
unsigned char Get_Analog_Value(No_MCU_Sensor *sensor, unsigned short *result)
{
